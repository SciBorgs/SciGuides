...
Refer to the [EnvironmentSetup](https://github.com/SciBorgs/SciGuides/blob/env-setup/reference-sheets/EnvironmentSetup.md) reference sheet to check if you have everything neccessary setup before starting this project. Feel free to ignore the normal vscode section as that won't be needed for this project. 
...
```java
  ...
  Another example: here, we test to see if our pose is changing as we drive. The `fastForward` allows some time to pass to give our drive time to move. Then we call our `getPose()` method and compare it to a new Pose2d. Which, if you should remember, returns a value of 0. So this test ensures that whenever our drive method is called, the robot doesn't stay at the 0 Pose2d but actually moves.
...
The last type of assert most commonly used in tests is the `assertEqual`. This simply tests if two values are the same. Both in assertEqual and assertNotEqual, you can pass in a tolerance value that will pass the test if the two different values are within the tolarence. This is usually done by having a `final double DELTA` value set to your prefered tolarence.
...
We use NetworkTables to make logging specific variables or objects easier. Read the [Telemetry doc]() for specifics on logging with NTables. We’re going to use the `@Log.NT` annotation as it allows us to log key metrics in real-time while the robot is running, which we can then view on dashboards like Shuffleboard or SmartDashboard.
...
### Continuing on
Once you feel ready for the next step, get started on an ArmBot project. Its going to have a different code structure than we discussed, but many of the ideas still remain. You can get started [here]().
